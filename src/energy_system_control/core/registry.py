...
class SimulationData:
    ...
    def create_empty_datasets(self, time_vector, signal_registry_ports, signal_registry_controllers, signal_registry_sensors):
        self.ports = np.empty((len(time_vector), len(signal_registry_ports._col_to_key)), dtype=float)
        self.controllers = np.empty((len(time_vector), len(signal_registry_controllers._col_to_key)), dtype=float)
        self.sensors = np.empty((len(time_vector), len(signal_registry_sensors._col_to_key)), dtype=float)
